...
#define PORT_MASK                                0x01 //to choose the port you need, only 1 port
  ...
  int num_etherType = 0;
	...
	if(lcore_id == master_core_id)
	{
		...
		while(1)
		{
          ...
          //Rio here
          /*
          I need the number of unique ether types of the received packets
          print out the ethertype if it is not the standard IPV4 packets, https://en.wikipedia.org/wiki/EtherType========

          And  the source IP address and destination IP address of each packet if the ethertype is 0x8000
          wait I'll send you screen...if you don't want to read the wikipedia link
          */
					struct sniff_ethernet *ethernet = (struct sniff_ethernet*) packet;
					if(ntohs(ethernet->ether_type) != 0x0800)
					{
						printf("The ether_type of the packet is %x \n", ntohs(ethernet->ether_type));
            num_etherType = num_etherType + 1;
						printf("The number of unique ether types of the received packets are %u",num_etherType);
					}
          if(ntohs(ethernet->ether_type) = 0x8000)
          {
            printf("The destination and the source address is %x \n ", ntohs(ethernet->ether_type))
          }
				}
